...
Alunos: Luiz Carlos dos Santos Júnior (20213008611)
...
int main(){
    ...
    TipoLista *lista = (TipoLista*) malloc(linhas*sizeof(TipoLista));
    ...
    for(int l=0; l<linhas; l++){
        ...
        */
        ...
        lista[l].Resultado = 0; //Padrao nao encontrado
        ...
        while (aponta != NULL){ // enquato a lista nao acabar
            ...
            if (ordem == 5)
            {
            lista[l].Resultado = 1; //Pista normal
        ...
        int ordemCont = 0;
        int ordemFaixa[13]={1,3,1,3,1,3,1,3,1,3,1,3,1};
        TipoApontador aponta2;
        aponta2 = lista[l].Primeiro->Prox;
        
        while (aponta2 != NULL){ // enquato a lista nao acabar
            if(aponta2->Item.Tipo == ordemFaixa[ordemCont]){ // verifica se é igual a sequencia
                ordemCont++;// proximo numero da sequencia
            }

            else{
                ordem = 0;
                if(aponta->Item.Tipo == ordemCerta[ordemCont]){ // apesar do numero nao ser igual a ordemCerta[ordem], 
                                                            // ele pode ser igual a ordemCerta[0]
                    ordemCont++;
                }
            }
            ...
            if (ordemCont == 13)
            {
            lista[l].Resultado = 2; //Pista com faixa
            break; // encontrou o padrao
            }

            aponta2 = aponta2->Prox;
        }
    }   
    ...
    for(int l=0; l<linhas; l++){
        if(lista[l].Resultado != 0){
            padrao++;

            if(lista[l].Resultado == 2){
                faixa = 1;
            }
        }
    }

    padrao = (padrao/linhas);
